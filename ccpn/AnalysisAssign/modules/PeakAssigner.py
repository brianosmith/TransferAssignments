...
#=========================================================================================
__copyright__ = "Copyright (C) CCPN project (https://www.ccpn.ac.uk) 2014 - 2026"
...
__dateModified__ = "$dateModified: 2026-03-17 16:48:20 +0000 (Tue, March 17, 2026) $"
__version__ = "$Revision: 3.5.0 $"
#=========================================================================================
# Created
#=========================================================================================
...
from ccpn.core.lib.AssignmentLib import nmrAtomsForPeaks, peaksAreOnLine, PROTEIN_NEF_ATOM_NAMES, NEF_ATOM_NAMES, \
    RNA_NEF_ATOM_NAMES, DNA_NEF_ATOM_NAMES
...
OtherByResType = f'{PULLDOWNPREFIX} Residue Specific {PULLDOWNPREFIX}'
OtherNames = f'{PULLDOWNPREFIX} All other atom-types {PULLDOWNPREFIX}'
OtherNonCanon = f'{PULLDOWNPREFIX} In this nmrResidue {PULLDOWNPREFIX}'
...
class PeakAssigner(CcpnModule):
    ...
    def _setWidgets(self):
        ...
        if self.includeSettingsWidget:
            ...
            row += 1
            self.correlatedDimsCheckBox = CheckBox(self.settingsWidget, checked=False,
                                                   grid=(row, 1), callback=self._updateInterface)
            Label(self.settingsWidget, "Correlated Dims", grid=(row, 0))

            row += 1
            self.autoCorrelateDimsCheckBox = CheckBox(self.settingsWidget, checked=False,
                                                      grid=(row, 1), callback=self._updateInterface)
            Label(self.settingsWidget, "Assign based on Correlated Dims", grid=(row, 0))
    ...
    def _updateTables(self, peaks):
        ...
        for dim, nmrAtoms in zip(range(Ndimensions), nmrAtomsForTables):
            ...
            if self.correlatedDimsCheckBox.isChecked():
                self.dimensionTabs[dim].setAlternativesTable(self._correlateDims(nmrAtomsForTables, dim))
            else:
                self.dimensionTabs[dim].setAlternativesTable(nmrAtomsForTables[dim])

        return _sizes

    def _checkCorrelatedDims(self) -> list | None:
        """
        Finds the valid magnetisation transfers for the current peaks spectra

        For a transfer to be valid the transfer type must be 'onebond', indirect and,
        dimension1 and dimension2 must not be 'Nh' or 'CO'
        """
        spectrum = self.current.peak.spectrum

        invalidDims = ['Nh', 'CO']

        magTransfers = spectrum.magnetisationTransfers
        specRefDims = spectrum.referenceExperimentDimensions

        validTransfer = [transfer for transfer in magTransfers
                         if not transfer.isIndirect and transfer.transferType == 'onebond'
                         and not (specRefDims[transfer.dimension1 - 1] in invalidDims
                                  and specRefDims[transfer.dimension2 - 1] in invalidDims)]

        return validTransfer

    def _correlateDims(self, atoms, dim, invert : bool = False) -> list | None:
        """
        Takes a list of atoms and a dim and finds the atoms that correlated based
        on valid magnetisationTransfers
        """

        if not (magTransfer := self._checkCorrelatedDims()):
            return atoms[dim]

        pairedAtoms = None

        transfersUsed = [True for transfer in magTransfer if dim in (transfer.dimension1 - 1, transfer.dimension2 - 1)]

        if not any(transfersUsed):
            return atoms[dim]

        for transfer in magTransfer:
            dim1 = transfer.dimension1 - 1
            dim2 = transfer.dimension2 - 1
            antiDim = dim1 if dim == dim2 else dim2

            if dim not in [dim1, dim2]:
                continue

            residues1Set = {atom.nmrResidue for atom in atoms[dim1]}
            residues2Set = {atom.nmrResidue for atom in atoms[dim2]}

            assignedAtomResidues = {atom.nmrResidue for atom in self.dimensionTabs[antiDim].tables[0].atomList}

            dim = antiDim if invert else dim

            commonResidues = residues1Set & residues2Set | assignedAtomResidues
            pairedAtoms = [atom for atom in atoms[dim] if atom.nmrResidue in commonResidues]
...
class AxisAssignmentObject(Frame):
    ...
    def __init__(self, parent, parentModule, dimIndex, mainWindow, grid=None, **kwds):
        ...
        self.split = Splitter(parent=self._assignmentsFrame, horizontal=False, acceptDrops=True)

        self.getLayout().addWidget(self.split)

        self.topSplit = Frame(None, setLayout=True, showBorder=True, acceptDrops=True)
        self.bottomSplit = Frame(None, setLayout=True, showBorder=True, acceptDrops=True)

        self.split.addWidget(self.topSplit)
        self.split.addWidget(self.bottomSplit)

        self._parent.setGuiNotifier(self.topSplit, [GuiNotifier.DROPEVENT], [DropBase.PIDS],
                                    callback=self._handleDropsFromSideBar)
        self._parent.setGuiNotifier(self.bottomSplit, [GuiNotifier.DROPEVENT], [DropBase.PIDS],
                                    callback=self._handleDropsFromSideBar)
    ...
    def _newNmrAtomPopup(self, mode=0):
        ...
        QtCore.QTimer.singleShot(0, self.chainPulldown.lineEdit().selectAll)
    ...
    def _assignNmrAtom(self, dim: int, action: bool = False, create: bool = True, nmrAtoms=None):
        ...
        try:
            ...
            with undoBlockWithoutSideBar():
                ...
                if self._parent.autoCorrelateDimsCheckBox.isChecked():
                    doubleTolerance = self._parent.doubleToleranceCheckbox.isChecked()
                    intraResidual = self._parent.intraCheckbox.isChecked()

                    validNmrAtoms = [nmrAtom for nmrAtom in self.project.nmrAtoms if
                                     not nmrAtom.nmrResidue.isDeleted or not nmrAtom.isDeleted]
                    nmrAtomsForTables = nmrAtomsForPeaks(self.current.peaks, validNmrAtoms,
                                                         doubleTolerance=doubleTolerance,
                                                         intraResidual=intraResidual)

                    correlatedAtoms, dims = self._findCorrelatedAtoms(nmrAtomsForTables, dim, True)
                    correlatedAtoms = correlatedAtoms or []
                    correlatedAtoms = [atom for atom in correlatedAtoms if atom.nmrResidue == nmrAtom.nmrResidue]

                try:
                    for peak in self.current.peaks:
                        if self._parent.autoCorrelateDimsCheckBox.isChecked() and correlatedAtoms:
                            for correlatedAtom in correlatedAtoms:
                                for corDim in dims:
                                    peakDimNmrAtoms = peak.dimensionNmrAtoms
                                    corDimNmrAtoms = list(peakDimNmrAtoms[corDim])

                                    if correlatedAtom not in corDimNmrAtoms:
                                        corDimNmrAtoms.append(correlatedAtom)

                                    allAtoms = list(peakDimNmrAtoms)
                                    allAtoms[corDim] = corDimNmrAtoms
    ...
    def _deassignNmrAtom(self, dim: int, nmrAtoms=None):
        ...
        try:
            ...
            correlatedAtoms = None

            if self._parent.autoCorrelateDimsCheckBox.isChecked():
                correlatedAtoms, dims = self._findCorrelatedAtoms(self.current.peak.dimensionNmrAtoms, dim, True)
                correlatedAtoms = correlatedAtoms or []
                correlatedAtoms = [atom for atom in correlatedAtoms if atom.nmrResidue == nmrAtom.nmrResidue]
                        ...
                        if self._parent.autoCorrelateDimsCheckBox.isChecked() and correlatedAtoms:
                            for correlatedAtom in correlatedAtoms:
                                for corDim in dims:
                                    ...
                                    if correlatedAtom in corDimNmrAtoms:
                                        corDimNmrAtoms.remove(correlatedAtom)
    ...
    def _findCorrelatedAtoms(self, atoms, dim, invert):
        """find the correlated atoms for asisgn/deassigning"""
        dimNmrAtoms = (self._parent._correlateDims(atoms, dim, invert=invert))
        magTransfers = self._parent._checkCorrelatedDims()

        dims = {mt.dimension2 - 1 if mt.dimension1 - 1 == dim else mt.dimension1 - 1 for mt in magTransfers
                if dim in (mt.dimension1 - 1, mt.dimension2 - 1)}

        return dimNmrAtoms, dims
    ...
    def _setAtomNames(self, nmrAtom=None, nmrResidue=None):
        ...
        if thisNmrResidueType:
            ...
            prot = PROTEIN_NEF_ATOM_NAMES.get(thisNmrResidueType, [])
            dna = DNA_NEF_ATOM_NAMES.get(thisNmrResidueType, [])
            rna = RNA_NEF_ATOM_NAMES.get(thisNmrResidueType, [])

            if atomsByResType := (prot + dna + rna):
            ...
            isoCode: str = isotopeCode or nmrAtom.isotopeCode
            # DT: this feels janky as it misses cases such as 'QA', 'QB' etc
            atomsByIsotopeCode = [atom for atom in isotopeCodeAtoms if ''.join(filter(str.isalpha, isotopeCode)) in atom]
            atomsByIsotopeCodeProt = NEF_ATOM_NAMES.get(isoCode) if isoCode in NEF_ATOM_NAMES else []

            atomsByIsotopeCode = OrderedSet(sorted(
                    (atomsByIsotopeCodeProt + atomsByIsotopeCode),
                    key=greekKey))

            atomOfSameIsotopeCode = isotopeCodeAtoms & atomsByIsotopeCode - thisNmrResAtoms
            atomInResidueSameCode = isotopeCodeAtoms & atomsByIsotopeCode & thisNmrResAtoms
            atomInResidueNonCanon = thisNmrResAtoms - isotopeCodeAtoms

            if thisNmrResAtoms:
                _atomNameOptions += ([OtherByResType] + list(atomInResidueSameCode))

            if atomInResidueNonCanon:
                _atomNameOptions += ([OtherNonCanon] + list(thisNmrResAtoms - isotopeCodeAtoms))
        ...
        self.atomTypePulldown.disableLabelsOnPullDown([OtherNames, OtherByIC, OtherByResType, OtherNonCanon])
